...
export const ingredientsProductRoviandaModalReducer = createReducer<ingredientsOfProductRoviandaModal[]>(initialValue,
    on(SAVE_ALL_INGREDIENTS_AVAIALBLE,(state,{products,ingredientsOriginal})=>{
        let idsToMark = ingredientsOriginal.map(x=>x.productId);
        console.log("STATE",products);
        let ingredientsMapped:ingredientsOfProductRoviandaModal[]=products.map((x)=>{
            console.log("x",x.id);
            return {
                productId:x.id,
