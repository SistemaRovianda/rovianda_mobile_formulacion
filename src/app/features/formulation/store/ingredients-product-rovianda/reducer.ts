...
export const ingredientProductRoviandaReducer = createReducer<
  ingredientsOfProductRovianda[]
>(
  initialValue,
  on(SET_INGREDIENTS, (state, { ingredients }) =>
    [...ingredients].map((x) => {
      return { ...x, original: true };
    })
  ),
  on(UPDATE_INGREDIENTS_DEL_BY_ID, (state, { ids }) => {
    return [...state].filter((x) =>
      !ids.includes(x.productId) ? true : false
    );
  })
);
