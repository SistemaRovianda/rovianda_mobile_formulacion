...
export class AddIngredientComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._store
      .select(GET_INGREDIENTS_AVAILABLES_STORE)
      .subscribe((ingredientsToModal) => {
        // obtiene todos los ingrediente secos del catalogo
        this.ingredientsToModal$ = ingredientsToModal;
        this._store
          .select(GET_INGREDIENTS_PRODUCT_ROVIANDA_STORE)
          .subscribe((ingredientsOfFormule) => {
            // obtiene los ingredientes para la formula
            this.ingredientsToUpdate = ingredientsOfFormule.map(
              (x) => x.productId
            );

            let productsMoldal = this.ingredientsToModal$.map(
              (x) => x.productId
            );
  ...
  createIngredients(
    ingredients: ingredientsOfProductRoviandaModal[]
  ): FormArray {
  ...
  getSelected(formValue) {
    ...
    this._store.dispatch(
      UPDATE_INGREDIENTS_DEL_BY_ID({ ids: [...idsToDelete] })
    );
    idsToDelete = [];
  ...
  addIngredients(formValue) {
    ...
    let ingredientsToAdd: ingredientsOfProductRovianda[] = this.ingredientsToModal$
      .filter((x, index) => formValue["ingredients"][index] == true)
      .map((x) => {
        return {
