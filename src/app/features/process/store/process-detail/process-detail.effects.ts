...
export class ProcessDetailEffect {
  ...
  loadLotsMeatProcess = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap((action) =>
        this.lotsMeatProcessService.getLotsMeatProcess().pipe(
  ...
  //     exhaustMap((action) =>
  //       this.processService
  //         .getProcessDetails(+localStorage.getItem("processId"))
  //         .pipe(
  //           switchMap((process: ProcessMetadata) => [
  //             fromProcessDetailActions.setProcessDetails({ process }),
  //           ])
  //         )
  //     )
  //   )
